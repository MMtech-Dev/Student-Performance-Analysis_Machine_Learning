...
def analyse_assessments():
    ...
    # df_clean['assessment_type'].value_counts().plot(kind='bar', ax=axes[0])
    # axes[0].set_title('Assessment Type Distribution')
    # axes[0].set_xlabel('Assessment Type')

    # # Weight distribution by type
    # sns.boxplot(data=df_clean, x='assessment_type', y='weight', ax=axes[1])
    # axes[1].set_title('Weight Distribution by Type')

    # # Assessment timeline
    # sns.scatterplot(data=df_clean, x='date', y='code_module', 
    ...
    # print(assessment_summary['Weight statistics'])
    # plt.tight_layout()
